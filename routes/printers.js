...
router.post('/add', async (req, res) => {
    try {
        ...
        var arrayOfInts = [0, 0, 0, 0, 0];
        const newPrinter = {
            location: typeof (location),
            url: typeof (req.body.url),
            model: typeof (req.body.model),
            tag: typeof (req.body.tag),
            contact: typeof (contactUser),
            toner: [0, 0, 0, 0, 0],
        ...
        const groupQuery = {
            groupName: typeof (req.body.group)
        };
...
router.get('/import', (req, res) => {
    res.sendFile(path_1.default.join(__dirname, '../public/samples', 'printers.csv'));
});
router.post('/import', async (req, res) => {
    try {
        ...
        const upload = await (0, csvtojson_1.default)().fromString(req.files.printerImport.data.toString('utf8'));
        for (let i = 0; i < upload.length; i++) {
            const locationVar = upload[i].location;
            const urlVar = upload[i].url;
            const tagVar = upload[i].service_tag;
            const p = {
                location: typeof (locationVar),
                url: typeof (urlVar),
                contact: String(),
                email: Boolean(),
                model: String(),
...
router.put('/:id/', async (req, res) => {
    try {
        ...
        const p = {
            email: String(),
            toner: typeof (req.body.toner),
            paper: typeof (req.body.paper),
            location: typeof (req.body.location),
            url: typeof (req.body.url),
            model: typeof (req.body.model),
            contact: String()
        };
